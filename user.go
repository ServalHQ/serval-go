...
type UserTeam struct {
	// Any of "TEAM_ROLE_UNSPECIFIED", "TEAM_ROLE_MANAGER", "TEAM_ROLE_AGENT".
	Role string       `json:"role"`
	Team UserTeamTeam `json:"team"`
	// JSON contains metadata for fields, check presence with [respjson.Field.Valid].
	JSON struct {
		Role        respjson.Field
		Team        respjson.Field
...
// Returns the unmodified JSON received from the API
func (r UserTeam) RawJSON() string { return r.JSON.raw }
func (r *UserTeam) UnmarshalJSON(data []byte) error {
	return apijson.UnmarshalRoot(data, r)
}

type UserTeamTeam struct {
	ID string `json:"id"`
	// A Timestamp represents a point in time independent of any time zone or local
	// calendar, encoded as a count of seconds and fractions of seconds at nanosecond
	// resolution. The count is relative to an epoch at UTC midnight on January 1,
...
// Returns the unmodified JSON received from the API
func (r UserTeamTeam) RawJSON() string { return r.JSON.raw }
func (r *UserTeamTeam) UnmarshalJSON(data []byte) error {
	return apijson.UnmarshalRoot(data, r)
}
